...
public class PropertyBasedSettingsTest {
  ...
  private static void experimentWith_splitter_characters_grouper_search_step() throws Exception {
    experimentWithSentenceSplitting("splitter_characters_grouper_search_step", 0.00005, 0.16,
        0.00005, false);
  }

  private static void experimentWith_splitter_characters_grouper_init_search_P_value()
      throws Exception {
    experimentWithSentenceSplitting("splitter_characters_grouper_init_search_P_value", 0., 1.,
        0.005, false);
  }

  private static void experimentWith_threshold_p_for_first_filter_separator_character()
      throws Exception {
    experimentWithSentenceSplitting("threshold_p_for_first_filter_separator_character", 0.3, 0.7,
        0.0005, false);
  }
  ...
  private static void experimentWith_threshold_p_for_second_filter_separator_character()
      throws Exception {
    experimentWithSentenceSplitting("threshold_p_for_second_filter_separator_character", 0., 0.9,
        0.005, false);
  }

  private static void experimentWithSentenceSplitting(final String keyName,
    ...
    for (Double splitter_characters_grouper_search_step = startValue;
         splitter_characters_grouper_search_step < endValue;
         splitter_characters_grouper_search_step += delta) {

      final String value = ((Object) (toInt ? splitter_characters_grouper_search_step.intValue() :
          splitter_characters_grouper_search_step)).toString();
      ...
      final Map<String, List<String>> testResult =
          SimpleSentenceSplitterCharactersExtractorQualityTest.executeTest();
      System.out.println(String.format("{\"value\": %f, \"errors\": %d},",
          splitter_characters_grouper_search_step,
          testResult.keySet().stream().mapToInt(key -> testResult.get(key).size()).sum()));
    }
    System.out.println("]");
  ...
  private static void experimentWithDictBuilding() throws Exception {
    ...
    for (Double word_set_dict_comparator_threshold = 0.;
      ...
      propertyBasedSettings
          .properties
          .setProperty(keyName, String.valueOf(word_set_dict_comparator_threshold));
  ...
  private static void experimentWithDictBuilding3D() throws Exception {
    ...
    for (Double recursive_substring_comparator_weight = 0.;
         recursive_substring_comparator_weight < 1.;
         recursive_substring_comparator_weight += 0.005) {

      for (Double simple_token_comparator_weight = 0.;
           simple_token_comparator_weight < 1.;
           simple_token_comparator_weight += 0.005) {

        propertyBasedSettings
            .properties
            .setProperty("recursive_substring_comparator_weight",
                String.valueOf(recursive_substring_comparator_weight));
        propertyBasedSettings
            .properties
            .setProperty("simple_token_comparator_weight",
                String.valueOf(simple_token_comparator_weight));
        final DictBuilderIntegTest.ExperimentResult experimentResult =
            DictBuilderIntegTest.runExperiment();
