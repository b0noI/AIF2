...
public class DictBuilderIntegTest {
  ...
  private static IdealDict loadIdealDict() throws IOException {
    try (final InputStream modelResource =
             DictBuilderTest.class.getResourceAsStream("ideal_dict.json")) {
  ...
  private static boolean rootTokenError(final IWord word, final IdealDict idealDict) {
    final Optional<Map.Entry<String, List<String>>> idealResult =
        idealDict.findTarget(word.getRootToken());
  ...
  private static int tokensErrors(final IWord word, final IdealDict idealDict) {
    ...
    return (int) word
        .getAllTokens()
        .stream()
        .filter(token -> !idealResult.get().getValue().contains(token)).count();
  }
