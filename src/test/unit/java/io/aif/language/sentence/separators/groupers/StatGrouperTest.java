...
public class StatGrouperTest {
  ...
  public void testGroup() throws Exception {
    // input arguments
    final List<String> inputTokens = Arrays.asList("token1", "token2", "token3");
    final List<Character> inputCharacters = Arrays.asList('.', ',');

    // mocks
    final List<String> filteredTokens = Arrays.asList("token1", "token2");
    final Map<Character, Integer> dotMap = new HashMap<Character, Integer>() {{
      put('A', 2);
      put('B', 3);
    }};
    final Map<Character, Map<Character, Integer>> mockCharactersConnections =
    ...
    final StatGrouper testInstance = new StatGrouper() {
      ...
      List<Set<Character>> convert(final List<CharactersGroup> groups) {
        assertEquals(groups, new ArrayList<CharactersGroup>() {{
          add(mockCharactersGroup);
        }});
  ...
  public void testConvert() throws Exception {
    // input arguments
    final Map<Character, Double> groupChracters = new HashMap<Character, Double>() {{
      put('.', 1.);
    }};
    final List<StatGrouper.CharactersGroup> inputCharactersGroups =
        new ArrayList<StatGrouper.CharactersGroup>() {{
      add(new StatGrouper.CharactersGroup(groupChracters, '.'));
    }};
  ...
  public void testAddCharactersToGroupWhenDifferentGroupExists() throws Exception {
    ...
    final StatGrouper.CharactersGroup commaGroup =
        new StatGrouper.CharactersGroup(commaCharacters, ',');
