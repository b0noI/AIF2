...
class RawGraphConverter {
  ...
  public static final Map<IWord, Map<IWord, Double>> recalculateConnections(
    ...
    distancesGraph.keySet().stream().forEach(key -> {
      ...
      distancesGraph
          ...
          .keySet()
          .forEach(innerKey ->
              results.get(key).put(innerKey,
                  distancesGraph.get(key).get(innerKey).stream().mapToDouble(x -> x).sum()));
    });

    final Double maxConnection = results.keySet().stream().mapToDouble(key -> {
      final OptionalDouble optMax =
          results.get(key).keySet().stream().mapToDouble(results.get(key)::get).max();
