...
public class TokensCountBasedWeightCalculator implements IVertexWeightCalculator<IWord> {
    ...
    @Override
    public Map<IWord, Double> calculate(final Map<IWord, Map<IWord, Double>> vertex, final Map<IWord, Long> count) {
        return vertex.keySet().stream().collect(Collectors.toMap(node -> node, node -> calculate(node)));
    }
