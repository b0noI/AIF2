...
import io.aif.algorithms.graph.fuzzy.similarity.GraphsEdgeSimilarityCalculator;
...
public class SynonymsFinder {
    ...
    public Set<Tuple2<IWord, IWord>> find(final AssociationGraph<IWord> text) {
        final ISimilarityCalculator<IWord> similarityCalculator = new GraphNodesSimilarityCalculator<IWord>(text, .75, .35);

        final List<Tuple2<Tuple2<IWord, IWord>, FuzzyBoolean>> results = text
               .getVertices()
               .stream()
               .flatMap(v -> text.getNeighbors(v).stream().map(n -> new Tuple2<IWord, IWord>(v, n)))
               .distinct()
                .filter(this::filter)
               .map(t -> new Tuple2<Tuple2<IWord, IWord>, FuzzyBoolean>(t, similarityCalculator.similar(t)))
                .sorted((tl, tr) ->
    ...
    private boolean filter(final IWord left, final IWord right) {
        ...
        final double diff =
                1. - Math.min(left.getCount(), right.getCount()) / Math.max(left.getCount(), right.getCount());
