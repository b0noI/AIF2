...
class HeuristicSentenceSplitter extends AbstractSentenceSplitter {
  ...
  public List<Boolean> split(final List<String> tokens,
                             final Map<ISeparatorGroupsClassifier.Group, Set<Character>>
                                 splitters) {
    final Map<ISeparatorGroupsClassifier.Group, Map<Character, Double>> connections =
        new HashMap<>();
    ...
    for (int i = 0; i < tokens.size() - 1; i++) {
      ...
      if (splitters.get(ISeparatorGroupsClassifier.Group.GROUP_1)
          .contains(currentToken.charAt(currentToken.length() - 1))) {
        connections
            .get(ISeparatorGroupsClassifier.Group.GROUP_1)
            .merge(nextFirstCharacter, 1., (v1, v2) -> v1 + v2);
      } else {
        connections
            .get(ISeparatorGroupsClassifier.Group.GROUP_2)
            .merge(nextFirstCharacter, 1., (v1, v2) -> v1 + v2);
      }
    }
    ...
    for (ISeparatorGroupsClassifier.Group group : ISeparatorGroupsClassifier.Group.values()) {
      ...
      final double sum =
          groupConnections.entrySet().stream().mapToDouble(Map.Entry::getValue).sum();
      groupConnections.keySet()
          .forEach(key -> groupConnections.put(key, groupConnections.get(key) / sum));
    }
    ...
    for (int i = 0; i < tokens.size() - 1; i++) {
      ...
      if (splitters.get(ISeparatorGroupsClassifier.Group.GROUP_1)
        ...
        if (!connections.get(ISeparatorGroupsClassifier.Group.GROUP_2)
            .keySet().contains(nextFirstCharacter)) {
          booleans.add(true);
        } else {
          final double isSep =
              connections.get(ISeparatorGroupsClassifier.Group.GROUP_1).get(nextFirstCharacter);
          final double isNotSep =
              connections.get(ISeparatorGroupsClassifier.Group.GROUP_2).get(nextFirstCharacter);
          if (isSep >= isNotSep) {
            booleans.add(true);
          } else {
            booleans.add(false);
